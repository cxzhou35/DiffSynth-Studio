...
def main(args):
    ...
    for image_path in tqdm(control_image_paths, desc="Inferring with Flux.1-dev-Kontext-LoRA-Finetune", total=len(control_image_paths)):
        control_image = Image.open(image_path)
        control_image = control_image.resize((width, height))
        # inference
        result_image = pipe(
            prompt=prompt,
            kontext_images=control_image,
        ...
        concat_image.paste(control_image, (0, 0))
